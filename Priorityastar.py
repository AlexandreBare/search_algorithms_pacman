...
initialNumFood = 0 #Initial number of food dots
...
class PacmanAgent(Agent):
    ...
    def astar(self, state):
        ...
        fringe.push((state, path, priorities))

        while True:
            if fringe.isEmpty():
                return []  # failure
                
            priority, (current, path, priorities) = fringe.pop() 
            #pops the priority value and a tuple 
            ...
            fringeKeys.discard(current_key)
            ...
            if current_key not in closed:
                ...
                for next_state, action in current.generatePacmanSuccessors():
                    ...
                    if next_key not in fringeKeys:
                        fringeKeys.add(next_key)
                        fringe.push((next_state, path + [action], priorities + [priority]))
                
        return path
                
    def anotherastar(self, state):
        """
        Given a pacman game state,
        returns a list of legal moves to solve the search layout.
        ...
        closed = []
        fringe = PriorityQueueWithFunction(PacmanAgent.priorityFunction)
        fringe.push((state, path, closed, priorities))
        #closed = set()
        ...
        while True:
            ...
            priority, (current, path, closed, priorities) = fringe.pop() #pops the priority value and a tuple 
                                                     #with the current state and the 
                                                     #resulting path
            if current.isWin():
                print(priorities)
                return path
            ...
            if current_key not in closed:
                ...
                for next_state, action in current.generatePacmanSuccessors():
                    fringe.push((next_state, path + [action], closed + [current_key], priorities + [priority]))
