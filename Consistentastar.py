...
class PacmanAgent(Agent):
    ...
    def priorityFunction(n):
        ...
        """
        global oldHeuristic
        if oldHeuristic > PacmanAgent.heuristic(n) + PacmanAgent.costFunction(n):
            print("Not Consistent")
        oldHeuristic = PacmanAgent.heuristic(n)
    ...
    def astar(self, state):
        ...
        path = [] #the list of positions pacman must follow
        closed = set() #the list of keys of each already visited game state
        fringe = PriorityQueueWithFunction(PacmanAgent.priorityFunction)
        #the priority queue that contains the next nodes to visit (the newly expanded nodes)
        ...
        while True:
            ...
            #pops the priority value and a tuple with the current state and path  
            
            if current.isWin():
                return path #returns the path when pacman wins the game

            current_key = key(current) #the position corresponding to the current state
            
            if current_key not in closed: #if the state has not already been visited
                closed.add(current_key)
                for next_state, action in current.generatePacmanSuccessors(): 
                #computes each new legal state and position
